...
public class GameScene extends PixelScene {
	...
	public synchronized void update() {
		...
		// Check for desync every 10 turns in LAN mode
	 ...
	 */
	public static void checkHashAndResync() {
		...
		try {
			...
			GLog.n("Hash exchange error: " + e.getMessage());
		}
	}

	/**
	 * Serializes the current dungeon state into a Bundle for network transmission.
	 */
	private static com.watabou.utils.Bundle serializeDungeonState() throws Exception {
		com.watabou.utils.Bundle bundle = new com.watabou.utils.Bundle();

		// Store key dungeon state
		bundle.put("seed", com.shatteredpixel.shatteredpixeldungeon.Dungeon.seed);
		bundle.put("depth", com.shatteredpixel.shatteredpixeldungeon.Dungeon.depth);
		bundle.put("branch", com.shatteredpixel.shatteredpixeldungeon.Dungeon.branch);
		bundle.put("gold", com.shatteredpixel.shatteredpixeldungeon.Dungeon.gold);
		bundle.put("energy", com.shatteredpixel.shatteredpixeldungeon.Dungeon.energy);

		// Store heroes and current hero index
		bundle.put("heroes", com.shatteredpixel.shatteredpixeldungeon.Dungeon.heroes);
		bundle.put("hero", com.shatteredpixel.shatteredpixeldungeon.Dungeon.hero);

		// Store level state
		if (com.shatteredpixel.shatteredpixeldungeon.Dungeon.level != null) {
			bundle.put("level", com.shatteredpixel.shatteredpixeldungeon.Dungeon.level);
		}
	...
	/**
	 * Restores the dungeon state from a serialized Bundle.
	 */
	private static void deserializeDungeonState(com.watabou.utils.Bundle bundle) throws Exception {
		// Restore key dungeon state
		com.shatteredpixel.shatteredpixeldungeon.Dungeon.seed = bundle.getLong("seed");
		com.shatteredpixel.shatteredpixeldungeon.Dungeon.depth = bundle.getInt("depth");
		com.shatteredpixel.shatteredpixeldungeon.Dungeon.branch = bundle.getInt("branch");
		com.shatteredpixel.shatteredpixeldungeon.Dungeon.gold = bundle.getInt("gold");
		com.shatteredpixel.shatteredpixeldungeon.Dungeon.energy = bundle.getInt("energy");

		// Restore heroes
		Object heroesObj = bundle.get("heroes");
		if (heroesObj instanceof java.util.ArrayList) {
			com.shatteredpixel.shatteredpixeldungeon.Dungeon.heroes =
				(java.util.ArrayList<com.shatteredpixel.shatteredpixeldungeon.actors.hero.Hero>)heroesObj;
		}

		// Restore current hero
		Object heroObj = bundle.get("hero");
		if (heroObj instanceof com.shatteredpixel.shatteredpixeldungeon.actors.hero.Hero) {
			com.shatteredpixel.shatteredpixeldungeon.Dungeon.hero =
				(com.shatteredpixel.shatteredpixeldungeon.actors.hero.Hero)heroObj;
		}

		// Restore level
		Object levelObj = bundle.get("level");
		if (levelObj instanceof com.shatteredpixel.shatteredpixeldungeon.levels.Level) {
			com.shatteredpixel.shatteredpixeldungeon.Dungeon.level =
				(com.shatteredpixel.shatteredpixeldungeon.levels.Level)levelObj;
		}
	}

	/**
	 * Handles desynchronization by either sending (host) or receiving (client) the dungeon state.
	 */
	private static void handleDesync(long localHash, long peerHash, int turn) {
		GLog.w("Desync detected at turn %d: local=%d peer=%d", turn, localHash, peerHash);

		if (com.shatteredpixel.shatteredpixeldungeon.network.NetworkManager.isHostMode()) {
			try {
				com.watabou.utils.Bundle bundle = serializeDungeonState();
				byte[] bytes = com.watabou.utils.FileUtils.bundleToBytes(bundle);
				com.shatteredpixel.shatteredpixeldungeon.network.NetworkManager.sendResyncBundle(bytes);
				GLog.p("Resync complete");
			} catch (Exception e) {
				GLog.n("Resync failed: " + e.getMessage());
				com.shatteredpixel.shatteredpixeldungeon.ShatteredPixelDungeon.reportException(e);
			}
		} else {
			try {
				byte[] bytes = com.shatteredpixel.shatteredpixeldungeon.network.NetworkManager.receiveResyncBundle();
				com.watabou.utils.Bundle bundle =
					com.watabou.utils.Bundle.read(new java.io.ByteArrayInputStream(bytes));
				deserializeDungeonState(bundle);
				GLog.p("Resync complete");
			} catch (Exception e) {
				GLog.n("Resync failed: " + e.getMessage());
				com.shatteredpixel.shatteredpixeldungeon.ShatteredPixelDungeon.reportException(e);
